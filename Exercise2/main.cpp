...
Vector2d qrsolve(const Matrix2d A, const Vector2d b) // la funzione calcola la soluzione del sistema con il metodo qr, valido per qualsiasi matrice
{
...
{
    Vector2d x;
    x << A.partialPivLu().solve(b);  // usa il pivoting per colonne (più lento ma più preciso, ok per matrici di dimensioni piccole)
    return x;
}
